...
public class MaximiseSatisfaction {
	...
	public double computeFitness(NRP nextRelease) {
		Double satisfaction = nextRelease.getCustomers().stream()
				.mapToDouble(c -> c.getImportance() * calculateSatisfaction(c))
				.sum();
	...
	public Double calculateSatisfaction(Customer customer) {
		List<Valuation> directValuations = customer.getAssigns().stream()
				.filter(v -> v.getContributesTo().isEmpty())
				.collect(Collectors.toList());

		List<Valuation> maxDirectValuations = selectMaximalValuations(directValuations, customer);
	...
	public Double calculateFulfillment(Requirement requirement, Customer customer) {
		Optional<RequirementRealisation> maxRealisation = requirement.getRealisations().stream()
				.filter(this::isImplemented)
				.max(Comparator.comparing(RequirementRealisation::getPercentage));
    
		Double maxPercentage = 0.0d;
	    if (maxRealisation.isPresent()) {
	    		maxPercentage = maxRealisation.get().getPercentage();
	    }

	    List<Valuation> dependencyValuations = getDependencyValuations(requirement, customer);

	    double aggregatedDepFulfillments = 0.0d;
	    if (!dependencyValuations.isEmpty()) {
	    	List<Valuation> maxDependencyValuations = selectMaximalValuations(dependencyValuations, customer);
	    	aggregatedDepFulfillments = calcOverallFulfillmentContribution(maxDependencyValuations, customer);
	    }
	
	    return Math.max(maxPercentage, aggregatedDepFulfillments);
	}
	...
	public List<Valuation> getDependencyValuations(Requirement requirement, Customer customer) {
		return requirement.getCombines().stream()
				.filter(v -> v.getAssignedBy().stream().anyMatch(c -> c.equals(customer)))
				.collect(Collectors.toList());
	}
	 ...
	 */	
	public List<Valuation> selectMaximalValuations(List<Valuation> valuations, Customer customer) {
		return valuations.stream()
	        .filter(v1 -> !valuations.stream().anyMatch(v2 -> !v2.equals(v1)
	        			&& v1.getRequirement().equals(v2.getRequirement())
	        			&& (v1.getValue() < v2.getValue() || (v1.getValue() == v2.getValue() & v1.hashCode() > v2.hashCode()))
	        		))
	        .collect(Collectors.toList());
	}
	...
	public Double calcOverallFulfillmentContribution(List<Valuation> valuations, Customer customer) {
	    ...
	    Double sumOfWeights = valuations.stream().mapToDouble(v -> v.getValue()).sum();
	
	    if (sumOfWeights == 0.0d) {
	    	throw new IllegalArgumentException("The sum of valuation values may not be 0.");
	    }
	
	    Double fullfillments = valuations.stream()
	...
	public boolean isImplemented(RequirementRealisation realisation) {
		return realisation.getDependsOn().stream()
				.allMatch(sa -> isSelected(sa));
	}

	public boolean isSelected(SoftwareArtifact softwareArtifact) {
		return softwareArtifact.getSolutions().size() > 0 && softwareArtifact.getRequires().stream().allMatch(this::isSelected);
	}
